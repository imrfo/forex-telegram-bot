...
# خواندن کلیدها از متغیرهای مخزن GitHub Secrets
...
def classify_news(title):
    ...
    # رد کردن خبرهای صرفاً سهام بدون ارتباط با ارز، کالا یا کریپتو
...
def extract_image_url(item):
    """استخراج دقیق تصویر شاخص خبر از متادیتاها و تگ‌های RSS"""
    # 1. media_content
    ...
    # 4. جستجو داخل HTML بدنه خبر برای تگ img
    raw_html = item.get("summary", "")
    if "content" in item and len(item.content) > 0:
        raw_html += " " + item.content[0].get("value", "")

    img_matches = re.findall(r'<img[^>]+src=["\'](https?://[^"\']+)["\']', raw_html, re.IGNORECASE)
...
# =========================
# Gemini AI (Analysis & Post Generator)
# =========================
...
۱. از ایموجی‌های متناسب با تم و جهت بازار (🟢/📈 برای روند صعودی یا مثبت، 🔴/📉 برای نزولی یا منفی، 📊 برای تکنیکال، ⚠️ برای هشدار) در متن استفاده کن.
۲. اگر در متن اعداد، سطوح حمایتی، مقاومتی یا اندیکاتورها ذکر شده، در بخش تحلیل تکنیکال به آن اشاره کن.
...
[ایموجی مناسب جهت بازار] <b>[تیتر جذاب، ژورنالیستی و فوری به فارسی]</b>

📌 <b>رویداد و خلاصه خبر:</b>
[۲ تا ۳ جمله شفاف از اصل اتفاق]

📊 <b>دیدگاه تحلیلی و تکنیکال:</b>
[نکات تکنیکال، وضعیت شکست سطوح یا رفتار قیمتی بر اساس متن خبر]

💡 <b>اثر روی نمادها:</b>
[یک خط درباره جفت‌ارزها یا دارایی‌های تحت تاثیر مثل طلا، دلار، نفت و...]

⚠️ <i>سلب مسئولیت: این تحلیل صرفاً جنبه اطلاع‌رسانی دارد و به منزله سیگنال خرید یا فروش نیست. مدیریت ریسک را همیشه رعایت کنید.</i>
...
def send_to_telegram(text, image_url=None):
    # اگر تصویر موجود بود، با sendPhoto ارسال شود
    if image_url:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto"
        payload = {
            "chat_id": TELEGRAM_CHAT_ID,
            "photo": image_url,
            "caption": text[:1024],  # سقف مجاز کپشن در تلگرام
            "parse_mode": "HTML"
        }
        res = requests.post(url, json=payload, timeout=15)
        if res.status_code == 200:
            return True
        print(f"Failed to send with photo, falling back to text. Error: {res.text}")

    # در صورت نبود تصویر یا خطا در بارگذاری عکس، ارسال متنی انجام می‌شود
...
def main():
    ...
    processed_count = 0
    MAX_PER_RUN = 2  # ارسال ۲ خبر جدید در هر نوبت جهت مدیریت بهینه سهمیه رایگان
    ...
    for item in reversed(feed.entries):
        ...
        try:
            ...
            if image_url:
                ...
                print("No image found, proceeding with text-only.")
